...
import { captureNativeReplyMarkdown } from "./chatgpt-native-copy";
...
export function setupReplyCopyButtonHandler(): void {
  document.addEventListener(
    ...
    async (event: MouseEvent) => {
      ...
      const container = findReplyContainer(button);
      if (!container) return;
      const copyScope = button.closest("section") ??
        button.closest("[data-conversation-screenshot-content]") ??
        container;

      // Prevent ChatGPT's handler immediately. We will invoke that same button
      // synthetically, read its original Markdown, then write it back after
      // conversion. This keeps the user gesture attached to the native action.
      event.preventDefault();
      event.stopPropagation();

      try {
        const nativeCopy = await captureNativeReplyMarkdown(copyScope);
        if (!("markdown" in nativeCopy)) {
          showToast(
            nativeCopy.reason === "copy_button_missing"
              ? "未找到 ChatGPT 原生复制按钮"
              : "无法读取 ChatGPT 原生复制内容",
            { type: "error", x: event.clientX, y: event.clientY }
          );
          return;
        }

        await navigator.clipboard.writeText(nativeCopy.markdown);
        showToast("已复制", { type: "success", x: event.clientX, y: event.clientY });
      } catch (error) {
        console.warn("[GPT Markdown] Native reply copy failed:", error);
        // The click was intentionally intercepted, so report the failure.
